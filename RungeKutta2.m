...
function [Volumen,OD,DBO] = RungeKutta(Q_e,Q_s,Ka,ODs,Kbdo,Ko2,ODe,DBOe,Vo,ODo,DBOo,h,t)
  ...
  while tiempo < t-1
    fV(i)= Q_e(i) - Q_s(i);
    q1v(i)= h*fV(i);
    fV(i+1)= Q_e(i+1) - Q_s(i+1);
    q2v(i)= h*fV(i+1)+q1v(i);
    ...
    q1od(i) = h*fOD(i);
    q1dbo(i) = h*fDBO(i);
    fOD(i+1) = (((Q_e(i+1)*ODe - Q_s(i+1)*OD(i))/Volumen(i+1)) + (Ka*(ODs - OD(i))) - (Kbdo*((OD(i)^2)/((OD(i)^2)+(Ko2))))*DBO(i));
    fDBO(i+1) = (((Q_e(i+1)*DBOe - Q_s(i+1)*DBO(i))/Volumen(i+1)) - (Kbdo*((OD(i)^2)/((OD(i)^2)+(Ko2))))*DBO(i));
...
[Volumen2, OD2, DBO2] = RungeKutta(Q_e,Q_s,Ka,ODs,Kbdo,Ko2,ODe,DBOe,x0(1),x0(2),x0(3),1,364);
#t= 0:1:364;
#plot(t,Volumen2);
#xlabel('Tiempo [dias]','fontsize',14);
#ylabel('Volumen [m^3/dia] ','fontsize',14);
